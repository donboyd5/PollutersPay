---
title: "Reports for individual entities - new data"
...
dca_base <- entities %>%
  left_join(coalcement_17502018 %>% select(uid, contains("share")),
            by="uid") %>%
  mutate(dca_base=value * (1 - coal_share - cement_share))

udca_base2 <- udca_base %>%
  select(uid_old=uid, dca_base) %>%
  left_join(xwalk %>% select(uid, uname, uid_old), by = "uid_old") %>%
  select(uid, uname, dca_base)
  

tmp <- dca_base %>%
  select(uid, uname, dca_base) %>%
  mutate(src="new") %>%
  bind_rows(udca_base2 %>% mutate(src="old")) %>%
  pivot_wider(names_from = src, values_from = dca_base) %>%
  mutate(diff=new - old)
