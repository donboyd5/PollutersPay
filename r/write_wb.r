...
fnotes <- function(wb, sname, years, threshold, exclusion){
  note_intro <- "Notes:"
...
#.. full report ---
makewb <- function(tottax, dca_base, nexus, years, threshold, exclusion, coal_entities, bankrupt, gtots, outpath){
  ...
  wb <- fnotes(wb, sname, years, threshold, exclusion)
  wb <- style(wb, sname)
