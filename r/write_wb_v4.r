...
fnotes <- function(wb, sname, data, threshold, exclusion, droplowgdp){
  years <- 2000:2018
  dropnote <- "  * Excludes State-Operated Entities in countries with low GDP per capita."
  keepnote <- "  * Does not exclude State-Operated Entities in countries with low GDP per capita."
  
  note_intro <- "Notes:"
  note1 <- paste0("  * Historical emissions period is ", years[1], "-", years[length(years)], ".")
  note2 <- paste0("  * Only includes entities where emissions were at least ", threshold, " MtCO2.")
  note3 <- paste0("  ** Excludes the first ", exclusion, " MtCO2 of emissions.")
  note4 <- ifelse(droplowgdp, dropnote, keepnote)
  ...
  writeData(wb, sheet = sname, x=note4, 
            startCol=1, startRow=note_start + 4, rowNames = FALSE, colNames = FALSE)
  return(wb)
}
...
makewb <- function(taxbase, outpath){
  ...
  # prep data ----
  threshold <- taxbase$threshold[1]
  exclusion <- taxbase$exclusion[1]
  droplowgdp <- taxbase$droplowgdp[1]
  
  data <- taxbase %>%
    mutate(blank1="", blank2="") %>%  # so we can have blank columns
    # use either ongc or taxbase for emit
    select(table_name, group, emit=ongc, blank1, pct_taxbase, blank2, tax_annual)  
  
  # initialize ----
  shellfile <- "PPCompanyShell_v5.xlsx"
  ...
  wb <- fnotes(wb, sname, data, threshold, exclusion, droplowgdp)
  wb <- style(wb, sname)
