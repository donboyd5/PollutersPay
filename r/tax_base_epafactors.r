...
# write workbooks ----
source(here::here("r", "write_wb_v2.r"))

sufx <- "_v4.xlsx"
gtot <- gff %>% filter(fuel=="total") %>% .$gvalue
...
makewb(tottax=dca_total_annual, dca_base, nexus, threshold=0.0005, exclusion="none",
       coal_entities, bankrupt, gtot, outpath)
openXL(outpath)
...
fname <- paste0(fbase, sufx)
outpath <- here::here("results", fname)
makewb(tottax=dca_total_annual, dca_base, nexus, threshold=0.0015, exclusion="none",
       coal_entities, bankrupt, gtot, outpath)
openXL(outpath)
...
fname <- paste0(fbase, sufx)
outpath <- here::here("results", fname)
makewb(tottax=dca_total_annual, dca_base, nexus, threshold=0.0015, exclusion="threshold",
       coal_entities, bankrupt, gtot, outpath)
openXL(outpath)
